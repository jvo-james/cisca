...
async function renderCalendar() {
  ...
  for (let index = 0; index < 42; index += 1) {
    ...
    cells.push(`
      <button type="button"
        class="calendar-day ${outside ? 'outside' : ''} ${closed ? 'closed' : ''} ${value === selectedDate ? 'selected' : ''} ${ymd(today) === value ? 'today' : ''}"
        data-date="${value}"
        ${disabled ? 'disabled' : ''}
...
async function uploadInspoFiles() {
  if (!uploadInput.files?.length || uploadedInspo.length) return;

  const files = [...uploadInput.files];
  if (files.length > 4) {
    toast('Please choose up to four photos.');
    uploadInput.value = '';
    return;
  }
  ...
  uploadStatus.textContent = 'Uploading your photos...';
  ...
  for (const file of files) {
    ...
    uploadStatus.textContent = `${uploadedInspo.length} of ${files.length} photos uploaded.`;
    form.inspoImages.value = JSON.stringify(uploadedInspo);
  }

  uploadStatus.textContent = uploadedInspo.length
    ? `${uploadedInspo.length} photo${uploadedInspo.length === 1 ? '' : 's'} ready for your booking.`
    : '';
}

function renderPreview() {
  if (!previewEl || !uploadInput) return;
  const files = [...uploadInput.files].slice(0,4);
  previewEl.innerHTML = files.map(file => {
    const url = URL.createObjectURL(file);
    return `<figure><img src="${url}" alt=""><figcaption>${file.name}</figcaption></figure>`;
  }).join('');

  if (files.length) {
    uploadStatus.textContent = `${files.length} photo${files.length === 1 ? '' : 's'} selected. They upload when you continue.`;
    uploadedInspo = [];
    form.inspoImages.value = '';
  } else {
...
async function goNext() {
  ...
  if (stage === 3 && uploadInput.files?.length && !uploadedInspo.length) {
