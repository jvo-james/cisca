import { IMAGES } from './images.js';const looks=[
  ['images/look-01.webp','Soft Glam','Birthday soft glam','soft-glam','Birthday'],
  ['images/look-02.webp','Collectionorial','Polished editorial finish','bold-glam','Photoshoot'],
  ...
  [IMAGES.looks[5],'Soft Glam','Cisca signature soft glam','soft-glam','Just because'],
  ['images/look-07.webp','Collectionorial','Camera-ready beauty','bold-glam','Photoshoot'],
  ['images/look-08.webp','Soft Glam','Evening glam','soft-glam','Birthday'],
  [IMAGES.looks[8],'Brows','Defined brow finish','brow-shaping','Just because'],
...
function render(){
  ...
  grid.innerHTML=rows.map(x=>`<a class="look-item" href="booking.html?service=${encodeURIComponent(x[3])}&occasion=${encodeURIComponent(x[4])}"><img src="${x[0]}" alt="${x[2]}" loading="lazy"><div class="look-caption"><span>${x[1]}</span><h3>${x[2]}</h3><span>Book this look <i class="fa-solid fa-arrow-right" aria-hidden="true"></i></span></div></a>`).join('');
